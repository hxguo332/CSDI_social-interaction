...
#SBATCH -t 24:00:00
#SBATCH --array=0-1
#SBATCH -o ./srun_logs/ablation_%A_%a.out
#SBATCH -e ./srun_logs/ablation_%A_%a.err

set -euo pipefail

# ============================================================
# Ablation study runner
# 4 Slurm array tasks = 4 scenarios in parallel.
# Each task runs 7 ablation variants sequentially on one GPU.
# ============================================================
...
for VARIANT in "${VARIANTS[@]}"; do
    CFG_NAME="generated_ablation/ablation_${SCENARIO}_${VARIANT}_e${EPOCHS}_len${DATA_LENGTH}_n${NSAMPLE}.yaml"
...
# The runner also sets these according to --model_variant.
# They are repeated here so that the saved config is explicit.
if '${VARIANT}' == 'baseline':
...
PY
    ...
    echo "Running ${VARIANT} on scenario ${SCENARIO}"
    ...
    echo "Finished ${VARIANT} on ${SCENARIO}: $(date)"
done

echo "============================================================"
echo "All variants finished for scenario ${SCENARIO}"
echo "Finished: $(date)"
echo "============================================================"
