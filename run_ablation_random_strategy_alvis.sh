...
#SBATCH -t 24:00:00
#SBATCH --array=0-1
#SBATCH -o ./srun_logs/random_strategy_%A_%a.out
#SBATCH -e ./srun_logs/random_strategy_%A_%a.err
...
echo "============================================================"
echo "Random-strategy comparison job"
...
for VARIANT in "${VARIANTS[@]}"; do
    CFG_NAME="generated_ablation/random_strategy_${SCENARIO}_${VARIANT}_e${EPOCHS}_len${DATA_LENGTH}_n${NSAMPLE}.yaml"

    echo "------------------------------------------------------------"
    echo "Preparing random-strategy config: ${CFG_NAME}"
...
# keep the selected module variant, but change the task setting to random completion.
...
PY
    ...
    echo "Running random-strategy ${VARIANT} on scenario ${SCENARIO}"
    ...
    "$PYTHON" exe_simulation_scenmap.py \
        ...
        --eval_collision \
        --force_random_strategy \
        --scenarios "${SCENARIO}"

    echo "Finished random-strategy ${VARIANT} on ${SCENARIO}: $(date)"
done

echo "============================================================"
echo "All random-strategy variants finished for scenario ${SCENARIO}"
echo "Finished: $(date)"
echo "============================================================"