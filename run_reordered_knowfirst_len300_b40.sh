...
#SBATCH -t 48:00:00
#SBATCH --array=0-3
#SBATCH -o ./srun_logs/reordered_knowfirst_len300_b40_%A_%a.out
#SBATCH -e ./srun_logs/reordered_knowfirst_len300_b40_%A_%a.err
...
POINTWISE_COLLISION_WEIGHT=0.5
CLEARANCE_LOSS_WEIGHT=0.1
PATH_COLLISION_LOSS_WEIGHT=0.2
...
# task/baseline and goal_full are identical to legacy task/full for this strategy.
...
echo "============================================================"
echo "Reordered ablation: know-first target + know-first missing | len300 b40"
...
for VARIANT in "${VARIANTS[@]}"; do
    CFG_NAME="generated_ablation/reordered_knowfirst_len300_b40_${SCENARIO}_${VARIANT}_e${EPOCHS}_len${DATA_LENGTH}_n${NSAMPLE}.yaml"
...
PY
    ...
    echo "Running reordered know-first ${VARIANT} on ${SCENARIO}"
    ...
    "$PYTHON" exe_simulation_scenmap.py \
        ...
        --ablation_order reordered \
        --max_neighbors "${MAX_NEIGHBORS}" \
        --eval_collision \
        --scenarios "${SCENARIO}"

    echo "Finished reordered know-first ${VARIANT} on ${SCENARIO}: $(date)"
done
